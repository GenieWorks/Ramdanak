...
public class UpdatesCrawler {
    ...
    private void updateShowTable() {
        ...
        showUpdatedQuery.setTrace(true);
        showUpdatedQuery.findInBackground(new FindCallback<ParseObject>() {
            @Override
            public void done(List<ParseObject> list, ParseException e) {
                if (e == null) {
                    MergeShowData mergeShowData = new MergeShowData();
                    mergeShowData.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, list);
                    Log.d(TAG, "success");
                } else {
    ...
    private class MergeShowData extends AsyncTask<List<ParseObject>, Void, Void> {

        @SafeVarargs
        @Override
        protected final Void doInBackground(List<ParseObject>... params) {
            List<ParseObject> list = (List<ParseObject>) params[0];
    ...
    private void updateChannelTable() {
        ...
        channelUpdatedQuery.findInBackground(new FindCallback<ParseObject>() {
            ...
            public void done(List<ParseObject> list, ParseException e) {
                if (e == null) {
                    MergeChannelData mergeChannelData = new MergeChannelData();
                    mergeChannelData.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, list);
                } else {
    ...
    private class MergeChannelData extends AsyncTask<List<ParseObject>, Void, Void> {

        @SafeVarargs
        @Override
        protected final Void doInBackground(List<ParseObject>... lists) {
            ...
            for (ParseObject parseObject : feed.values()) {
                ...
                channel.setDescription("removed");
                channel.setName(name);
                channel.setRating_1(rating1);
                channel.setRating_2(rating2);
                channel.setRating_3(rating3);
    ...
    private void updateTVRecord() {
        ...
        updatedQuery.findInBackground(new FindCallback<ParseObject>() {
            ...
            public void done(List<ParseObject> list, ParseException e) {
                if (e == null) {
                    MergeRecordsData mergeRecordsData = new MergeRecordsData();
                    mergeRecordsData.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, list);
                } else {
    ...
    private class MergeRecordsData extends AsyncTask<List<ParseObject>, Void, Void> {

        @SafeVarargs
        @Override
        protected final Void doInBackground(List<ParseObject>... lists) {
            List<ParseObject> list = (List<ParseObject>) lists[0];
